      ...
      <motion.div variants={container} initial="hidden" animate="show" className="max-w-4xl mx-auto space-y-6">
        ...
        {activeDelivery && (
          <motion.div variants={item}>
            <div className="bg-white border-2 border-[#00D4AA] rounded-card shadow-[0_0_20px_rgba(0,212,170,0.15)] p-4 flex items-center gap-4">
        ...
        {isLoading ? (
          ...
          <motion.div variants={item} className="space-y-3">
            {data.map((pv) => (
                <motion.div key={pv.prescription_id} whileHover={{ x: 2 }}>
                  <div className="bg-white border border-[#A7F3D0] rounded-card shadow-soft p-4 space-y-3">
          ...
          <motion.div variants={item}>
            <div className="bg-[#F0FDF9] border border-[#A7F3D0] rounded-card shadow-soft p-8 text-center">
              ...
              <Link href="/patient/prescription/new"
                className="inline-flex px-5 py-2 rounded-btn text-sm font-medium text-white bg-[#00D4AA] hover:bg-[#009B7D] transition-colors duration-200">
        ...
        <motion.div variants={item} className="pt-4">
          <button onClick={() => window.dispatchEvent(new CustomEvent("toast", { detail: { type: "warning", message: t(locale, "patient.toast.report") } }))}
            className="text-sm text-gray-500 hover:text-[#FF4D6D] transition-colors duration-200 underline underline-offset-2">
